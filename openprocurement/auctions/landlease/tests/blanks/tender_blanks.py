...
def edit_role(self):
    ...
    if SANDBOX_MODE:
        fields.add('procurementMethodDetails')
    self.assertEqual(set(self.auction._fields) -
                     self.auction._options.roles['edit_active.tendering'].fields, fields)
...
def create_auction_invalid(self):
    ...
    self.assertEqual(response.json['errors'], [{u'description': {u'endDate': [
                     u"Could not parse invalid_value. Should be ISO8601."]}, u'location': u'body', u'name': u'enquiryPeriod'}])
    ...
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'],
                     [{u'description': [u'rectificationPeriod.endDate should come at least 5 working days earlier than tenderPeriod.endDate'],
                       u'location': u'body',
                       u'name': u'rectificationPeriod'},
                      {u'description': [u'tenderPeriod should be greater than 6 days'],
                         u'location': u'body',
        ...
        auction_data['auctionPeriod'] = {
            'startDate': (
                ...
                timedelta(
                    days=10) /
                DEFAULT_ACCELERATION).isoformat()}
        auction_data['rectificationPeriod'] = {
            'endDate': (
                now +
                timedelta(
                    days=9) /
...
@unittest.skipIf(
    get_now() < DGF_ADDRESS_REQUIRED_FROM,
    "Can`t create auction without item.address only from {}".format(DGF_ADDRESS_REQUIRED_FROM))
def required_dgf_item_address(self):
    ...
    response = self.app.post_json(
        '/auctions', {'data': auction_data}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'],
...
def create_auction_auctionPeriod(self):
    ...
    if SANDBOX_MODE:
        ...
        self.assertEqual(
            parse_date(
                auction['tenderPeriod']['endDate']).time(), time(
                20, 0))
...
def create_auction_rectificationPeriod_generated(self):
    ...
    self.assertEqual(
        parse_date(
            self.initial_data['auctionPeriod']['startDate']).date(), parse_date(
            auction['auctionPeriod']['shouldStartAfter'], TZ).date())
    timedelta_during_periods_ends = parse_date(
        auction['tenderPeriod']['endDate']) - parse_date(
        auction['rectificationPeriod']['endDate'])
    if not SANDBOX_MODE:
        self.assertEqual(
            timedelta_during_periods_ends,
            MINIMAL_PERIOD_FROM_RECTIFICATION_END)
    else:
        self.assertEqual(
            timedelta_during_periods_ends,
...
def create_auction_rectificationPeriod_set(self):
    ...
    auction = response.json['data']
    self.assertIn('rectificationPeriod', auction)
    timedelta_during_set_periods_ends = parse_date(
        auction['tenderPeriod']['endDate']) - parse_date(
        auction_data['rectificationPeriod']['endDate'])
    self.assertGreaterEqual(
        timedelta_during_set_periods_ends,
        MINIMAL_PERIOD_FROM_RECTIFICATION_END)
    self.assertEqual(
        timedelta_during_set_periods_ends,
        parse_date(
...
def additionalClassifications(self):
    ...
    response = self.app.post_json(
        '/auctions', {'data': auction_data}, status=201)
    data = response.json['data']
    self.assertEqual(
        data['items'][0]['additionalClassifications'][0]['scheme'],
        'CPVS')
    self.assertEqual(
    ...
    response = self.app.post_json(
        '/auctions', {'data': auction_data}, status=422)
    self.assertEqual(response.json['errors'], [{u'description': [{u'classification': {u'scheme': [
                     u"Value must be one of [u'CPV', u'CAV-PS']."]}}], u'location': u'body', u'name': u'items'}])
    ...
    self.assertRegexpMatches(
        response.json['errors'][0]['description'][0]['additionalClassifications'][0]['id'][0],
        "Value must be one of*")


@unittest.skipIf(
    get_now() < CLASSIFICATION_PRECISELY_FROM,
    "Can`t setup precisely classification from {}".format(CLASSIFICATION_PRECISELY_FROM))
def cavps_cpvs_classifications(self):
    ...
    self.assertEqual(response.json['errors'], [{u'description': [{u'classification': {u'id': [
                     u'At least CPV classification class (XXXX0000-Y) should be specified more precisely']}}], u'location': u'body', u'name': u'items'}])
...
def patch_auction(self):
    ...
    response = self.app.patch_json(
        '/auctions/{}?acc_token={}'.format(
            ...
            'data': {
                'value': {
                    'amount': auction['value']['amount'] - 80}}}, status=422)
    self.assertEqual(response.json['errors'],
                     [{'location': 'body',
                       'name': 'minimalStep',
                       'description': [u'value should be less than value of auction']}])
...
def patch_auction_rectificationPeriod_invalidationDate(self):
    ...
    self.assertNotIn(
        'invalidationDate',
        response.json['data']['rectificationPeriod'])
    auction = response.json['data']
    owner_token = response.json['access']['token']
...
def patch_old_auction_rectificationPeriod_invalidationDate(self):
    ...
    data = deepcopy(self.initial_data)
    data['auctionPeriod']['startDate'] = (
        get_now().date() + timedelta(days=8)).isoformat()
    data['guarantee'] = {"amount": 100, "currency": "UAH"}
...
def auction_Administrator_change(self):
    ...
    self.assertEqual(
        response.json['data']["procuringEntity"]["identifier"]["id"],
        "00000000")

    response = self.app.patch_json(
        '/auctions/{}/questions/{}'.format(
            auction['id'], question['id']), {
            "data": {
                "answer": "answer"}}, status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.post_json(
        '/auctions/{}/cancellations?acc_token={}'.format(
            auction['id'], auction_token), {
            'data': {
                'reason': 'cancellation reason', 'status': 'active'}})
    self.assertEqual(response.status, '201 Created')
    self.assertEqual(response.content_type, 'application/json')

    self.app.authorization = ('Basic', ('administrator', ''))
    response = self.app.patch_json(
        '/auctions/{}'.format(auction['id']), {'data': {'mode': u'test'}})
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['data']['mode'], u'test')
...
def patch_auction_denied(self):
    ...
    self.assertEqual(response.content_type, 'application/json')
    self.assertIn(
        u'Auction can be edited only during the rectification period',
...
def patch_auction_during_rectification_period(self):
    ...
    for param in ['title', 'title_en', 'title_ru']:
        ...
        self.assertEqual(
            response.json['data'][param],
            auction[param] + u' EDITED')
    ...
    for param in ['description', 'description_en', 'description_ru']:
        ...
        self.assertNotEqual(
            response.json['data']['items'][0][param],
            auction['items'][0][param])
        self.assertEqual(
            response.json['data']['items'][0][param],
            auction[param] + u' EDITED')
    ...
    self.assertEqual(response.content_type, 'application/json')
    self.assertNotEqual(
        response.json['data']['items'][0]['address'],
        auction['items'][0]['address'])
    self.assertEqual(
        response.json['data']['items'][0]['address'],
        address_data_edited)
    ...
    self.assertNotEqual(
        ...
        auction['items'][0]['classification'])
    self.assertEqual(
        response.json['data']['items'][0]['classification'],
        classification_data_edited)
    ...
    self.assertEqual(response.content_type, 'application/json')
    self.assertNotEqual(
        response.json['data']['items'][0]['unit'],
        auction['items'][0]['unit'])
    self.assertEqual(
        response.json['data']['items'][0]['unit'],
        unit_data_edited)
    ...
    self.assertEqual(response.content_type, 'application/json')
    self.assertNotEqual(
        response.json['data']['items'][0]['quantity'],
        auction['items'][0]['quantity'])
    self.assertEqual(
        response.json['data']['items'][0]['quantity'],
        auction['items'][0]['quantity'] + 1)
    ...
    self.assertEqual(response.content_type, 'application/json')
    self.assertNotEqual(
        response.json['data']['tenderAttempts'],
    ...
    self.assertEqual(
        response.json['data']['dgfID'],
        auction['dgfID'] + u'EDITED')
    ...
    response = self.app.patch_json(
        '/auctions/{}?acc_token={}'.format(
            ...
            'data': {
                'value': {
                    'valueAddedTaxIncluded': False, 'amount': auction['value']['amount']}, 'minimalStep': {
    ...
    self.assertEqual(response.json['data']['value']
                     ['valueAddedTaxIncluded'], False)
    self.assertEqual(
        response.json['data']['minimalStep']['valueAddedTaxIncluded'],
        False)
...
def one_valid_bid_auction(self):
    ...
    # set award as active
    self.app.patch_json('/auctions/{}/awards/{}?acc_token={}'.format(auction_id,
                                                                     award_id, owner_token), {"data": {"status": "active"}})
    # get contract id
...
def first_bid_auction(self):
    ...
    response = self.app.get(
        '/auctions/{}/awards?acc_token={}'.format(auction_id, owner_token))
    # get pending award
    award2 = [i for i in response.json['data']
              if i['status'] == 'pending.verification'][0]
    award2_id = award2['id']
...
@unittest.skipIf(
    not SANDBOX_MODE,
    'procurementMethodDetails is absent while SANDBOX_MODE is False')
